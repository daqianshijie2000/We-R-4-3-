...
The basic purpose of the chi-squared test used in the dataset is that we want to investigate whether the test of independence can be used to examine the relationship between the Status as the target variable or the binary variable (y) and other independent variables. We decided to select five independent variables, an applicant’s owned realty, an applicant’s total income, an applicant’s marital status, an applicant’s education status, and an applicant’s monthly balance due, to test if the relationship between the binary variable "Status" and other five variables are independent. 
...
# Conclusion:
<br>
After we tested different statistics methods in the dataset, below is our findings based on what we have been performacing in the dataset. 

* Balancing the dataset is required to create a well performing model. Without a balanced dataset, the model can undergo misclassification. 
